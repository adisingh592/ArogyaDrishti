"""
SehatSetu - Skin Cancer Detection Model Training Script
...
DATASET_CANDIDATES = [
    os.path.join(PROJECT_ROOT, "organized_datasets", "Oncology_Dermatology", "Skin_Cancer", "skin cancer"),
    os.path.join(PROJECT_ROOT, "organized_datasets", "Oncology_Dermatology", "Skin_Cancer")
]

MODEL_SAVE_DIR = os.path.join(PROJECT_ROOT, "SehatSetu", "models", "image", "skin_cancer")
RESULTS_SAVE_DIR = os.path.join(PROJECT_ROOT, "SehatSetu", "results", "image", "skin_cancer")
...
def load_datasets(dataset_path):
    print("=" * 60)
    print("SEHATSETU - SKIN CANCER IMAGE CLASSIFIER TRAINING")
    print("=" * 60)
...
def save_artifacts(model, history, y_true, y_pred, class_names):
    ...
    model_save_path = os.path.join(MODEL_SAVE_DIR, "skin_cancer_model.keras")
    model.save(model_save_path)
    print(f"Saved model to: {model_save_path}")
    
    metrics_path = os.path.join(RESULTS_SAVE_DIR, "metrics.txt")
    with open(metrics_path, "w", encoding="utf-8") as f:
        f.write("SEHATSETU - SKIN CANCER MODEL EVALUATION METRICS\n")
        f.write("=" * 50 + "\n\n")
    ...
    plt.title("Confusion Matrix - MobileNetV2 (Skin Cancer)")
    ...
    print("\nSkin Cancer image training workflow completed successfully!")
