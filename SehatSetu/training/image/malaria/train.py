"""
SehatSetu - Malaria Cell Detection Model Training Script
...
DATASET_CANDIDATES = [
    os.path.join(PROJECT_ROOT, "organized_datasets", "Infectious_Diseases", "Malaria", "malaria", "cell_images", "cell_images"),
    os.path.join(PROJECT_ROOT, "organized_datasets", "Infectious_Diseases", "Malaria", "malaria", "cell_images"),
    os.path.join(PROJECT_ROOT, "organized_datasets", "Infectious_Diseases", "Malaria", "malaria")
]

MODEL_SAVE_DIR = os.path.join(PROJECT_ROOT, "SehatSetu", "models", "image", "malaria")
RESULTS_SAVE_DIR = os.path.join(PROJECT_ROOT, "SehatSetu", "results", "image", "malaria")
...
def find_dataset():
    for path in DATASET_CANDIDATES:
        if os.path.exists(path):
            # Verify if subdirectories (Parasitized/Uninfected) exist here
            subdirs = [d for d in os.listdir(path) if os.path.isdir(os.path.join(path, d))]
            if "Parasitized" in subdirs or len(subdirs) >= 2:
                return path
...
def load_datasets(dataset_path):
    print("=" * 60)
    print("SEHATSETU - MALARIA CELL CLASSIFIER TRAINING")
    print("=" * 60)
...
def save_artifacts(model, history, y_true, y_pred, class_names):
    ...
    model_save_path = os.path.join(MODEL_SAVE_DIR, "malaria_model.keras")
    model.save(model_save_path)
    print(f"Saved model to: {model_save_path}")
    
    metrics_path = os.path.join(RESULTS_SAVE_DIR, "metrics.txt")
    with open(metrics_path, "w", encoding="utf-8") as f:
        f.write("SEHATSETU - MALARIA CELL MODEL EVALUATION METRICS\n")
        f.write("=" * 50 + "\n\n")
    ...
    plt.title("Confusion Matrix - MobileNetV2 (Malaria Cell Images)")
    ...
    print("\nMalaria cell image training workflow completed successfully!")
