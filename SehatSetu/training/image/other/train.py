"""
SehatSetu - Multi-Image Medical Classifier Training Script
...
# Default path checks across image datasets
DATASET_CANDIDATES = [
...
MODEL_SAVE_DIR = os.path.join(PROJECT_ROOT, "SehatSetu", "models", "image", "other")
RESULTS_SAVE_DIR = os.path.join(PROJECT_ROOT, "SehatSetu", "results", "image", "other")
...
def load_datasets(dataset_path):
    print("=" * 60)
    print("SEHATSETU - GENERIC MEDICAL IMAGE CLASSIFIER TRAINING")
    print("=" * 60)
...
def save_artifacts(model, history, y_true, y_pred, class_names):
    ...
    model_save_path = os.path.join(MODEL_SAVE_DIR, "other_image_model.keras")
    model.save(model_save_path)
    print(f"Saved model to: {model_save_path}")
    
    metrics_path = os.path.join(RESULTS_SAVE_DIR, "metrics.txt")
    with open(metrics_path, "w", encoding="utf-8") as f:
        f.write("SEHATSETU - GENERIC MEDICAL IMAGE MODEL EVALUATION METRICS\n")
        f.write("=" * 50 + "\n\n")
    ...
    print("\nImage training workflow completed successfully!")
