"""
SehatSetu - Thyroid Disease Model Training Script
...
DATASET_CANDIDATES = [
    os.path.join(PROJECT_ROOT, "organized_datasets", "Endocrine_Metabolic", "Thyroid_Disease", "thyroid+disease (1)", "hypothyroid.data"),
    os.path.join(PROJECT_ROOT, "organized_datasets", "Endocrine_Metabolic", "Thyroid_Disease", "thyroid+disease (1)", "sick-euthyroid.data"),
    os.path.join(PROJECT_ROOT, "organized_datasets", "Endocrine_Metabolic", "Thyroid_Disease", "thyroid+disease (1)", "allhypo.data")
]

MODEL_SAVE_DIR = os.path.join(PROJECT_ROOT, "SehatSetu", "models", "tabular", "thyroid")
RESULTS_SAVE_DIR = os.path.join(PROJECT_ROOT, "SehatSetu", "results", "tabular", "thyroid")
...
def find_dataset():
    for path in DATASET_CANDIDATES:
        if os.path.exists(path):
            return path
    raise FileNotFoundError("Thyroid dataset not found in organized_datasets.")
...
def load_data():
    ...
    print("=" * 60)
    print("SEHATSETU - THYROID DISEASE MODEL TRAINING")
    print("=" * 60)
...
def preprocess_data(df):
    ...
    # Target is first column in UCI hypothyroid.data
    target_col = df.columns[0]
    
    # Binary target: 1 = hypothyroid / sick, 0 = negative / normal
    y = df[target_col].astype(str).str.lower().apply(lambda x: 0 if "negative" in x else 1).values
    
    X = df.drop(columns=[target_col])
    
    # One-hot encode categorical columns
    X_encoded = pd.get_dummies(X, drop_first=True)
    feature_names = [f"feature_{col}" for col in X_encoded.columns]
...
def save_artifacts(results_df, best_model_name, best_model, best_preds, y_test, preprocessor):
    ...
    model_path = os.path.join(MODEL_SAVE_DIR, "thyroid_model.pkl")
    joblib.dump(best_model, model_path)
    print(f"Saved model to: {model_path}")
    
    preproc_path = os.path.join(MODEL_SAVE_DIR, "thyroid_preprocessor.pkl")
    ...
    with open(metrics_path, "w", encoding="utf-8") as f:
        f.write("SEHATSETU - THYROID DISEASE CLASSIFICATION METRICS\n")
        f.write("=" * 50 + "\n\n")
    ...
    plt.title(f"Confusion Matrix - {best_model_name} (Thyroid Disease)")
    ...
    print("\nThyroid Disease training workflow completed successfully!")
