"""
SehatSetu - Diabetes Model Training Script
...
# Source Dataset in organized_datasets
DATASET_CANDIDATES = [
    os.path.join(PROJECT_ROOT, "organized_datasets", "Endocrine_Metabolic", "Diabetes", "diabetes health indicator", "diabetes_binary_5050split_health_indicators_BRFSS2015.csv"),
    os.path.join(PROJECT_ROOT, "organized_datasets", "Endocrine_Metabolic", "Diabetes", "diabetes health indicator", "diabetes_binary_health_indicators_BRFSS2015.csv"),
    os.path.join(PROJECT_ROOT, "organized_datasets", "Endocrine_Metabolic", "Diabetes", "diabetes+130-us+hospitals+for+years+1999-2008", "diabetic_data.csv")
]

MODEL_SAVE_DIR = os.path.join(PROJECT_ROOT, "SehatSetu", "models", "tabular", "diabetes")
RESULTS_SAVE_DIR = os.path.join(PROJECT_ROOT, "SehatSetu", "results", "tabular", "diabetes")
...
def find_dataset():
    for path in DATASET_CANDIDATES:
        if os.path.exists(path):
            return path
    raise FileNotFoundError("Diabetes dataset not found in organized_datasets.")
...
def load_data():
    ...
    print("=" * 60)
    print("SEHATSETU - DIABETES MODEL TRAINING")
    print("=" * 60)
...
def train_and_evaluate(X_train, X_test, y_train, y_test):
    ...
    # For fast execution on large datasets, limit SVM training sample if necessary
    if len(X_train) > 10000:
        svm_sample_idx = np.random.RandomState(42).choice(len(X_train), size=5000, replace=False)
        X_train_svm = X_train[svm_sample_idx]
        y_train_svm = y_train.iloc[svm_sample_idx] if hasattr(y_train, 'iloc') else y_train[svm_sample_idx]
    else:
        X_train_svm = X_train
        y_train_svm = y_train
...
def save_artifacts(results_df, best_model_name, best_model, best_preds, y_test, preprocessor):
    ...
    # 2. Save Preprocessor
    preproc_path = os.path.join(MODEL_SAVE_DIR, "diabetes_preprocessor.pkl")
    ...
    with open(metrics_path, "w", encoding="utf-8") as f:
        f.write("SEHATSETU - DIABETES CLASSIFICATION METRICS\n")
        f.write("=" * 50 + "\n\n")
    ...
    plt.title(f"Confusion Matrix - {best_model_name} (Diabetes)")
